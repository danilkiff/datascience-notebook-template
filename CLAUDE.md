...
Infrastructure files at the root; `workspace/` contains starter Python
code (training template, tests, flows) with tracked structure and
gitignored user content (data, models, custom notebooks).
...
Two networks: `backend` (postgres, mlflow, s3) and
`frontend` (jupyter, mlflow, s3).
MinIO (`s3` service) is in both networks — MLflow accesses it via backend,
...
- `Dockerfile.jupyter` — base: scipy-notebook, adds project deps from
  `requirements/jupyter.txt` (fastai, mlflow, torch, hydra, dvc, etc.)
...
- `requirements/jupyter.constraints` — version upper bounds for jupyter deps
- `copier.yml` — Copier template configuration
- `.devcontainer/` — VS Code Dev Container config (attaches to jupyter)
...
| compose-lint | compose, Dockerfiles, reqs, `.env.example` |
...
- Hardcode `shm_size` — use `SHM_SIZE` env var
  (default 2g; GPU overlay default 8g)
